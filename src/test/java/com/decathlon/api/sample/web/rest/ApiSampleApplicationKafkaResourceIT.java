package com.decathlon.api.sample.web.rest;
...
import com.decathlon.api.sample.IntegrationTest;
import com.decathlon.api.sample.config.EmbeddedKafka;
import com.decathlon.api.sample.config.KafkaSseConsumer;
import com.decathlon.api.sample.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class ApiSampleApplicationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/api-sample-application-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/api-sample-application-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/api-sample-application-kafka/unregister"));
                return;
            }
        }
