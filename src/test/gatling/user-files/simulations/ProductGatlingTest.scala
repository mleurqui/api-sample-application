...
class ProductGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Product entity")
        ...
        .repeat(2) {
            ...
            .get("/services/apisampleapplication/api/products")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new product")
            .post("/services/apisampleapplication/api/products")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "name":"SAMPLE_TEXT"
                }""")).asJson
            ...
            .repeat(5) {
                ...
                .get("/services/apisampleapplication${new_product_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created product")
            .delete("/services/apisampleapplication${new_product_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
